export const COMMAND_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    audience: 'business',
    capability: {
      id: 'ecommerce-warehouse-relations',
      name: '发现可查询的数据仓库表',
      description: '列出当前数据库账号具有 SELECT 权限的业务表和视图，不暴露未授权关系。',
      examplePrompt: '公司数据库里我可以查询哪些表？',
      requiredInputs: ['已配置的数据仓库权限'],
      optionalInputs: ['schema', '表名关键词'],
      delivery: '可查询的 schema、表或视图名称及说明',
      commandTemplate: 'tbcli db tables [--schema <NAME>] [--keyword <TEXT>] --json',
    },
  },
  {
    key: 'db describe',
    ...
    audience: 'business',
    capability: {
      id: 'ecommerce-warehouse-relation-schema',
      name: '查看数据仓库表结构',
      description: '查看当前账号获准读取的一个业务表或视图的字段、类型和约束。',
      examplePrompt: '查看成本版本表有哪些字段和关联关系',
      requiredInputs: ['schema.table'],
      optionalInputs: [],
      delivery: '字段、数据类型、空值约束、默认值和主外键约束',
      commandTemplate: 'tbcli db describe --relation <schema.table> --json',
    },
  },
  {
    key: 'db sql',
    ...
    audience: 'business',
    capability: {
      id: 'ecommerce-warehouse-readonly-sql',
      name: '跨表查询公司电商数据仓库',
      description: '由 Agent 根据实时表结构组织参数化只读 SQL，查询当前账号获准读取的表和视图。',
      examplePrompt: '查询成本主数据里这个商家编码的全部历史版本',
      requiredInputs: ['业务问题', '已配置的数据仓库权限'],
      optionalInputs: ['筛选条件', '排序', '返回数量'],
      delivery: '结构化只读查询结果、行数、截断状态和查询摘要',
      commandTemplate: 'tbcli db tables --json；tbcli db describe --relation <schema.table> --json；tbcli db sql --sql-file <FILE> [--params-json <JSON_ARRAY>] --json',
    },
  },
  ...
  { key: 'profit actual coverage', maturity: 'stable', audience: 'business', capability: {id:'profit-actual-coverage',name:'盘点实际利润数据缺口',description:'按支付月和次月15日退款截止口径，只读核对订单、实际退款、实际快递费、成本、推广费与负责人；未匹配运单按已确认的每单2元规则估算并披露，不执行利润计算。',examplePrompt:'盘点杉居旗舰店2026年7月正式利润还缺什么数据',requiredInputs:['店铺键','利润月份'],optionalInputs:['利润政策版本'],delivery:'可计算状态、阻断缺口、提醒缺口、缺失对象清单及实际/估算金额',commandTemplate:'tbcli profit actual coverage --shop-key <KEY> --month <YYYY-MM> [--policy-version <VERSION>] --json'} },
  { key: 'profit actual query', maturity: 'stable', audience: 'business', capability: {id:'profit-actual-query',name:'查询月度实际利润',description:'按支付月、次月15日退款截止、实际快递账单和已确认政策，只读计算全店、负责人及商品实际利润；报告模式一次返回三级分析数据。',examplePrompt:'查询杉居旗舰店2026年7月全店、负责人和商品实际利润',requiredInputs:['店铺键','利润月份'],optionalInputs:['利润政策版本','负责人','汇总粒度'],delivery:'全店利润摘要、负责人利润、商品盈亏、实际与估算费用及质量提示',commandTemplate:'tbcli profit actual query --shop-key <KEY> --month <YYYY-MM> [--group-by shop|owner|product|day|report] --json'} },
  { key: 'profit actual audit-cost', maturity: 'stable', audience: 'business', capability: {id:'profit-actual-cost-audit',name:'核对商品订单行成本',description:'按商品逐订单行核对旺店通总成本、旺店通SKU单位成本乘数量和已批准成本主数据单位成本乘数量，全程只读。',examplePrompt:'核对这个商品7月每条订单明细的goodsCost是否等于SKU单位成本乘数量',requiredInputs:['店铺键','利润月份','商品ID'],optionalInputs:[],delivery:'成本来源覆盖、逐订单行三方成本、差额和不一致明细',commandTemplate:'tbcli profit actual audit-cost --shop-key <KEY> --month <YYYY-MM> --product-id <ID> --json'} },
  { key: 'product images validate', maturity: 'stable', audience: 'business', capability: {id:'product-image-mapping-validate',name:'校验商品主图映射表',description:'校验商品ID、商品标题、HTTPS图片链接、类目和商品ID唯一性，不修改数据库。',examplePrompt:'先检查这份商品ID和商品图片映射表能否入库',requiredInputs:['Excel映射表','店铺键','店铺名称'],optionalInputs:[],delivery:'行数、唯一商品数、文件摘要和逐项错误',commandTemplate:'tbcli product images validate --input <FILE> --shop-key <KEY> --shop-name <NAME> --json'} },
  { key: 'product images import', maturity: 'stable', audience: 'business', capability: {id:'product-image-mapping-import',name:'导入商品主图映射',description:'以商品ID为唯一键幂等写入权威商品主图映射；再次导入会更新变化的标题、图片链接和类目。',examplePrompt:'把最新的商品ID图片映射表导入数据库并更新变化的主图',requiredInputs:['Excel映射表','店铺键','店铺名称','数据库维护权限'],optionalInputs:[],delivery:'新增、更新、未变化数量和来源文件摘要',commandTemplate:'tbcli product images import --input <FILE> --shop-key <KEY> --shop-name <NAME> --json'} },
  {
    key: 'sycm catalog',
    maturity: 'stable',
    audience: 'business',

...
- **Unsupported `--no-sandbox` banner:** treat this as an unsafe or outdated tbcli browser launch. Stop the login attempt. For an npm installation, run `npm install -g @petercjl/tbcli@0.5.1`, verify `tbcli --version` is at least `0.5.1`, close that browser, and rerun `tbcli auth login`. For a source checkout, use its repository update workflow instead of replacing it with npm. If the Agent uses a managed Skill copy, inspect `tbcli skill status --agent <agent>` and run `tbcli skill update --agent <agent>` after the CLI upgrade; linked Skills update with the package. Never advise repeated slider attempts or disabling the Chrome sandbox.
